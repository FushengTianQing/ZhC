...
**当前状态**：`python -m zhpp hello.zhc` 走 AST 路径直接生成 C 代码，无语义验证。
...
**成功标准**：
1. `python -m zhpp hello.zhc` 自动执行语义验证
...
修改 `src/zhpp/cli.py` 的 `_compile_ast()` 方法（第127-172行），在 `parse_source()` 之后、`CCodeGenerator()` 之前插入语义验证调用。
...
### 目标
用户运行 `python -m zhpp hello.zhc` 时，编译器自动执行语义验证，报告符号表和作用域层面的错误。
...
python -c "
from zhpp.semantic import SemanticAnalyzer, SemanticError
...
PYTHONPATH=src python -m zhpp /tmp/test_ok.zhc -v

# 重复定义应报错
echo '整数型 主函数() { 整数型 x = 1; 整数型 x = 2; 返回 0; }' > /tmp/test_err.zhc
PYTHONPATH=src python -m zhpp /tmp/test_err.zhc -v 2>&1 | grep "重复定义"
```
...
```bash
cd /Users/yuan/Projects/zhc/
PYTHONPATH=src python -m zhpp --help | grep "skip-semantic"
```
...
**文件**：`src/zhpp/semantic/type_utils.py`（**新建**）
**预计行数**：~100行
**优先级**：P0
...
from zhpp.semantic.type_utils import type_name_to_typeinfo
...
**测试策略**：使用子进程调用 `python -m zhpp`，检查 stderr 输出中的错误信息。
...
def run_compiler(code: str, extra_args=None) -> tuple:
    ...
    cmd = [sys.executable, "-m", "zhpp", tmp_file, "-v"]
    if extra_args:
...
PYTHONPATH=src python -m zhpp /tmp/err.zhc
...
# ===== M2 =====
# T2.1 完成后
PYTHONPATH=src python -c "from zhpp.semantic.type_utils import type_name_to_typeinfo; print(type_name_to_typeinfo('整数型'))"
...
# 覆盖率
python -m pytest tests/test_phase5_semantic.py --cov=zhpp.semantic --cov-report=term-missing
```
