...
class TestInterproceduralAnalyzer(unittest.TestCase):
    ...
    def test_call_graph_construction(self):
        ...
        functions = [
            ...
            {"name": "计算", "params": ["n"], "return_type": "整数型", "body": []},
        ]
...
class TestPointerAnalyzer(unittest.TestCase):
    ...
    def test_null_pointer_detection(self):
        ...
        # 应该检测到空指针解引用
        null_issues = [
            i for i in issues if i.error_type == PointerError.NULL_DEREFERENCE
        ]
    ...
    def test_dangling_pointer_detection(self):
        ...
        # 应该检测到悬空指针解引用
        dangling_issues = [
            i for i in issues if i.error_type == PointerError.DANGLING_DEREFERENCE
        ]
    ...
    def test_double_free_detection(self):
        ...
        # 应该检测到双重释放
        double_free_issues = [
            i for i in issues if i.error_type == PointerError.DOUBLE_FREE
        ]
    ...
    def test_null_check_propagation(self):
        ...
        self.assertTrue(
            len(null_issues) == 0
            or all(i.severity in ["warning", "info"] for i in null_issues)
        )
...
# 注意: TestIntegration 使用旧版 SemanticAnalyzer API（analyze_variable_decl, report 等），
