...
class TestSymbolTablePerformance:
    ...
    def test_cache_effectiveness(self, optimized_table):
        ...
        for i in range(50):
            symbol = Symbol(
                name=f"var_{i}", symbol_type="变量", data_type="整数型", is_defined=True
            )
        ...
        cache_hit_rate = (
            float(stats["lookup_stats"]["cache_hit_rate"].rstrip("%")) / 100
        )
    ...
    def test_scope_chain_optimization(self, optimized_table):
        ...
        scope_chain_ratio = (
            scope_chain_lookups / total_lookups if total_lookups > 0 else 0
        )
        ...
        assert (
            scope_chain_ratio <= 0.5
        ), f"作用域链查找比例应 ≤ 50%（当前: {scope_chain_ratio:.1%})"
    ...
    def test_full_benchmark(self):
        ...
        result = benchmark_symbol_lookup(
            iterations=1000, scope_depth=10, symbols_per_scope=50
        )
