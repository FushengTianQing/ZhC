...
from zhc.analyzer.interprocedural_alias import (
...
class TestAllocationSite:
    ...
    def test_allocation_site_creation(self):
        ...
        site = analyzer.new_allocation_site(
            function="test_func", line=10, var_name="x", is_heap=False
        )
    ...
    def test_heap_allocation_site(self):
        ...
        site = analyzer.new_allocation_site(
            function="main", line=5, var_name="ptr", is_heap=True
        )
...
class TestPointerOperations:
    ...
    def test_pointer_assign(self):
        ...
        assert (
            func_info.points_to["ptr1"].targets == func_info.points_to["ptr2"].targets
        )
...
class TestCallSiteAliasing:
    ...
    def test_call_propagates_alias(self):
        ...
        # main 调用 foo(q)
        analyzer.add_call(caller="main", callee="foo", line=10, arg_mapping={"p": "q"})
    ...
    def test_return_value_alias(self):
        ...
        assert any(
            s.var_name == "global_var" for s in main_info.points_to["result"].targets
        )
...
class TestEdgeCases:
    ...
    def test_self_call(self):
        ...
        analyzer.add_call(
            caller="recursive", callee="recursive", line=10, arg_mapping={"n": "n"}
        )
