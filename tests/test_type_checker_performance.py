...
class TestTypeCheckerPerformance:
    ...
    def test_type_lookup_performance(self):
        ...
        # 验收标准：性能差异在 ±20% 范围内（缓存开销可接受）
        assert -20 <= improvement <= 20, f"性能差异 {improvement:.1f}% 超出可接受范围"
    ...
    def test_binary_op_check_performance(self):
        ...
        # 原版本
        time_original = self.measure_time(
            lambda: [checker_original.check_binary_op(1, op, t1, t2) 
        ...
        time_cached = self.measure_time(
            lambda: [checker_cached.check_binary_op_cached(1, op, t1, t2, expr_sources[i]) 
    ...
    def test_cache_stats(self):
        ...
        # 执行一些操作
        for i in range(10):
            checker_cached.check_binary_op_cached(1, "+", int_type, float_type, f"expr{i}")
    ...
    def test_cache_clear(self):
        ...
        # 添加一些缓存
        for i in range(5):
            checker_cached.check_binary_op_cached(1, "+", int_type, int_type, f"expr{i}")
        ...
        def run_cached():
            for i, op in enumerate(test_operations):
                if op[0] == "lookup":
                    ...
                    checker_cached.check_binary_op_cached(1, op[1], op[2], op[3], f"expr{i}")
