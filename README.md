...
[![测试](https://img.shields.io/badge/测试-3511%20passed-brightgreen)](tests/)
[![覆盖率](https://img.shields.io/badge/覆盖率-57.80%25-yellow)]()
[![综合评分](https://img.shields.io/badge/评分-B%2B%207.68%2F10-blue)]()

> ZhC 是一个用 Python 实现的**中文语法 C 编译器**，将使用中文关键字的 C 代码编译为 LLVM IR，最终输出原生机器码。核心理念是**完全中文友好编程**——258 个中文关键字覆盖 C 语言全部功能。
>
> 在一个月内（2026-03 ~ 2026-04）从 v1 迭代到 v10，实现了完整的编译流水线、异常处理、闭包协程、智能指针、反射系统、泛型、模式匹配等高级特性。综合评分 **B+ (7.68/10)**。

| 维度 | 数据 |
|------|------|
| 源文件 | **323** 个 Python 文件 + **39** 个 C/H 运行时文件 |
| 代码量 | **~135,000** 行 Python |
| AST 节点 | **76** 种类型 |
| 类定义 | **272+** 个 |
| IR 操作码 | **60+** 个 |
| 中文关键字 | **258** 个（含前缀版 390 个） |
| 测试用例 | **3511** 个（3509 passed / 0 failed / 4 skipped） |
| 覆盖率 | **57.80%** |
...
```
v1.0 (2026-03)    → 基础结构 / Lexer / Parser
v2.0 (2026-03)    → 类语法 / 继承 / 虚函数 / 运算符重载
v3.0 (2026-03)    → 模块系统 / 导入导出 / 作用域管理
v4.0 (2026-03)    → 内存安全分析 / 智能指针
v5.0 (2026-04-07) → Week5 重构 / Dispatch Table / 状态机 / 命令模式
v6.0 (2026-04-08) → 统一 API / IR 后端 / 配置分组 / 覆盖率 57%
v7.0 (2026-04-09) → 异常处理 / 闭包 / 协程 / 网络库
v8.0 (2026-04-10) → 泛型系统 G.01~G.07 / 内存管理全完成
v9.0 (2026-04-11) → 模式匹配 M.01~M.09 / 测试 3511 passed
v10.0 (当前)       → 反射系统 (P5) / 完整特性集
```
...
| 文档 | 内容 |
|------|------|
| [architecture.md](architecture.md) | 系统架构详解 (302 行) |
...
```
======================== 3511 tests collected =========================
✅ 3509 passed (含 3 xpassed)
⏭️  4 skipped
📊 覆盖率: 57.80%
```
...
| 依赖 | 版本 | 用途 |
|------|------|------|
| **Python** | >=3.10 (推荐 3.14.3) | 编译器实现语言 |
| **llvmlite** | >=0.47.0 | LLVM IR 绑定，原生代码生成（唯一生产依赖） |
| **clang 或 gcc** | - | 编译生成的 C 代码 |

开发依赖: Black (格式化) / Ruff (Lint) / MyPy (类型检查) / pytest (测试) / pre-commit (Git Hooks)
