...
使用示例:
  ...
  系统工具:
    ...
    zhc info                           # 显示系统信息
        ...
        init_parser.add_argument('--template', choices=['console', 'library', 'module'], 
                                default='console', help='项目模板')
        
        # new 命令
        new_parser = subparsers.add_parser('new', help='创建新模块')
        new_parser.add_argument('module_name', help='模块名称')
        new_parser.add_argument('--type', choices=['util', 'data', 'service', 'ui'], 
        ...
        build_parser.add_argument('--verbose', '-v', action='store_true', help='详细输出')
        
        # run 命令
        run_parser = subparsers.add_parser('run', help='编译并运行')
        run_parser.add_argument('file', nargs='?', help='运行文件（不指定则运行主程序）')
        run_parser.add_argument('--args', help='程序参数')
        
        # test 命令
        test_parser = subparsers.add_parser('test', help='运行测试')
        test_parser.add_argument('--module', help='测试特定模块')
        test_parser.add_argument('--coverage', action='store_true', help='生成测试覆盖率报告')
        
        # doc 命令
        doc_parser = subparsers.add_parser('doc', help='生成文档')
        doc_parser.add_argument('--open', action='store_true', help='生成后打开文档')
        doc_parser.add_argument('--format', choices=['html', 'markdown', 'pdf'], 
                               default='html', help='文档格式')
...
''', encoding='utf-8')
        ...
        readme_file.write_text(f'''# {project_dir.name}
...
''', encoding='utf-8')
        ...
        # 主程序
        main_file = project_dir / 'src' / '主程序.zhc'
        main_file.write_text('''导入 工具::数学工具;
