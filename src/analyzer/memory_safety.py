...
class NullPointerChecker:
    ...
    def track_allocation(self, var_name: str, line: int, size: int = 1, is_array: bool = False):
        """跟踪内存分配"""
    ...
    def verify_access(self, var_name: str, operation: str, line: int) -> Optional[SafetyIssue]:
        """验证内存访问"""
        # 检查是否已分配
...
class BoundsChecker:
    ...
    def check_access(self, var_name: str, index: int, operation: str, line: int) -> Optional[SafetyIssue]:
        """检查访问"""
...
class OwnershipTracker:
    ...
    def __init__(self):
        ...
        self.borrows: Dict[str, List[Tuple[str, int, bool]]] = {}  # var_name -> [(borrower, line, is_mutable)]
    
    def declare_owner(self, var_name: str, scope: str):
        """声明所有者"""
        self.ownerships[var_name] = scope
    
    def borrow(self, var_name: str, borrower: str, line: int, mutable: bool = False) -> Optional[SafetyIssue]:
        """借用检查"""
        ...
        # 检查是否已有可变借用
        for existing_borrower, existing_line, existing_mutable in self.borrows[var_name]:
            if existing_mutable:
                return SafetyIssue(
                    level=SafetyLevel.UNSAFE,
...
class LifetimeAnalyzer:
    ...
    def __init__(self):
        ...
        self.borrow_lifetimes: Dict[str, List[Tuple[int, int, str]]] = {}  # borrower -> [(start, end, var_name)]
    ...
    def check_lifetime(self, borrower: str, var_name: str, use_line: int) -> Optional[SafetyIssue]:
        """检查生命周期有效性"""
        # 检查借用的变量是否存在
...
class RaceConditionDetector:
    ...
    def __init__(self):
        ...
        self.accesses: Dict[str, List[Tuple[str, int, bool]]] = {}  # var_name -> [(thread, line, is_write)]
    ...
    def detect_races(self) -> List[SafetyIssue]:
        ...
        for var_name, accesses in self.accesses.items():
            # 检查是否有并发写
            write_accesses = [(t, l) for t, l, w in accesses if w]
            read_accesses = [(t, l) for t, l, w in accesses if not w]
            
            # 如果有多个线程写入
            write_threads = set(t for t, l in write_accesses)
            ...
            # 如果有写和读冲突
            read_threads = set(t for t, l in read_accesses)
...
class StackAllocationAnalyzer:
    ...
    def __init__(self):
        self.stack_vars: Dict[str, Tuple[int, int, str]] = {}  # var_name -> (line, size, scope)
    ...
    def check_stack_overflow(self, threshold: int = 1024 * 1024) -> Optional[SafetyIssue]:
        """检查栈溢出风险"""
    ...
    def get_stack_report(self) -> str:
        ...
        for var_name, (line, size, scope) in sorted(self.stack_vars.items(), key=lambda x: -x[1][1]):
...
class MemorySafetyAnalyzer:
    ...
    def analyze_function(self, func_name: str, statements: List[dict]) -> dict:
        ...
        for stmt in statements:
            ...
            if 'alloc' in stmt_type or '新建' in stmt_type:
                ...
                result['allocations'].append({'var': var_name, 'line': line, 'size': size})
            ...
            # 追踪访问
                ...
                self.use_after_free_checker.track_pointer_flow(var_name, line, stmt_type)
            ...
            # 追踪赋值（借用）
                ...
                if target and target != var_name:
                    ...
                    self.lifetime_analyzer.track_borrow_lifetime(target, line, scope_end, var_name)
        ...
        result['issues'].extend([
            {'type': issue.level.value, 'message': issue.message, 'line': issue.line_number}
    ...
    def generate_report(self) -> str:
        ...
        leak_count = len(self.leak_detector.check_leaks())
        ...
        if not self.all_issues:
            ...
            warning_count = sum(1 for i in self.all_issues if i.level == SafetyLevel.WARNING)
            unsafe_count = sum(1 for i in self.all_issues if i.level == SafetyLevel.UNSAFE)
