...
class TypeInfo:
    ...
    def __str__(self) -> str:
        ...
        if self.category == TypeCategory.ARRAY:
            ...
            return f"{self.base_type}{size_str}" if self.base_type else f"数组{size_str}"
    ...
    def equals(self, other: 'TypeInfo') -> bool:
        ...
        if self.category == TypeCategory.POINTER:
            return self.base_type and other.base_type and self.base_type.equals(other.base_type)
...
class TypeChecker:
    ...
    def check_type_decl(self, line: int, type_name: str, var_name: str) -> Optional[TypeInfo]:
        """
        检查类型声明
        ...
        if not type_info:
            self.errors.append((
                line,
                "未知类型",
                f"变量 '{var_name}' 使用了未知类型 '{type_name}'"
            ))
            return None
    ...
    def check_binary_op(
        ...
        # 比较运算符
            ...
            self.errors.append((
                line,
                "比较类型错误",
                f"无法比较类型 '{left_type}' 和 '{right_type}'"
            ))
            return None
