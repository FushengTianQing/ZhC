...
class ScopeChecker:
        ...
        for symbol in exited_scope.get_unused_symbols():
            if symbol.category == SymbolCategory.VARIABLE:
                self.warnings.append(
                    (symbol.line, "未使用变量", f"变量 '{symbol.name}' 已声明但未使用")
                )
    ...
    def declare_variable(
        ...
        # 检查变量遮蔽
        shadowed = (
            self.current_scope.parent.lookup(name)
    ...
    def declare_function(
        self, line: int, name: str, type_info: TypeInfo, is_static: bool = False
    ) -> bool:
        """
        声明函数
        ...
        if self.current_scope.level > 0:
            self.errors.append(
                (line, "函数声明位置错误", f"函数 '{name}' 只能在全局作用域声明")
            )
            return False
    ...
    def declare_parameter(self, line: int, name: str, type_info: TypeInfo) -> bool:
        """
        声明函数参数
        ...
        if self.current_scope.level != 1:
            self.errors.append(
                (line, "参数声明位置错误", f"参数 '{name}' 应该在函数作用域声明")
            )
            return False
        ...
        if existing:
            self.errors.append(
                (line, "重复参数", f"参数 '{name}' 已声明（行 {existing.line}）")
            )
            return False
    ...
    def lookup_symbol(self, line: int, name: str) -> Optional[Symbol]:
        ...
        if not symbol:
            self.errors.append((line, "未声明符号", f"符号 '{name}' 未声明"))
            return None
    ...
    def check_assignable(self, line: int, name: str) -> Optional[Symbol]:
        ...
        # 检查是否为常量
        if symbol.is_const:
            self.errors.append((line, "常量赋值", f"常量 '{name}' 不能被赋值"))
            return None
    ...
    def check_initialized(self, line: int, name: str) -> bool:
        ...
        if not symbol.is_initialized:
            self.warnings.append((line, "未初始化变量", f"变量 '{name}' 可能未初始化"))
            return False

        return True

    def declare_typedef(self, line: int, name: str, type_info: TypeInfo) -> bool:
        """
        声明类型定义
        ...
        if existing:
            self.errors.append(
                (line, "类型重定义", f"类型 '{name}' 已定义（行 {existing.line}）")
            )
            return False
    ...
    def declare_label(self, line: int, name: str) -> bool:
        ...
        if self.current_scope.level < 1:
            self.errors.append(
                (line, "标签位置错误", f"标签 '{name}' 只能在函数内部声明")
            )
            return False
        ...
        if existing and existing.category == SymbolCategory.LABEL:
            self.errors.append(
                (line, "重复标签", f"标签 '{name}' 已声明（行 {existing.line}）")
            )
            return False
