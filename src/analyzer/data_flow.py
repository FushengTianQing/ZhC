        ...
        for defn in self.definitions:
            if defn.line_number < use_line:
                ...
                for other_def in self.definitions:
                    if (other_def.line_number > defn.line_number and 
                        other_def.line_number < use_line):
                        is_killed = True
                        break
                if not is_killed:
...
class SymbolTable:
    ...
    def define(self, var_name: str, value: Optional[str] = None, source: str = "unknown"):
        """定义变量"""
    ...
    def is_defined(self, var_name: str) -> bool:
        ...
        return var_name in self.symbols and self.symbols[var_name] in (
            VarState.DEFINED, VarState.USED, VarState.CONSTANT, VarState.TAINTED
        )
...
class DataFlowAnalyzer:
    ...
    def build_def_use_chains(self, statements: List[dict]) -> Dict[str, DefUseChain]:
        ...
        for stmt in statements:
            ...
            if stmt_type in ('var_decl', 'assign'):
                ...
                chains[var_name].add_definition(Definition(
                    var_name=var_name,
                    line_number=line,
    ...
    def _extract_vars_from_expr(self, expr: str) -> List[str]:
        ...
        # 过滤关键字
        keywords = {'如果', '否则', '循环', '当', '返回', '整数型', '浮点型', 
                   '字符型', '布尔型', '空指针', '真', '假'}
    ...
    def propagate_constants(self, statements: List[dict]) -> Tuple[List[dict], Dict[str, str]]:
        """常量传播优化
            ...
            # 赋值传播污点
                ...
                for var_name in vars_in_value:
                    if var_name in tainted_vars:
                        ...
                        self.symbol_table.set_tainted(target, tainted_vars[var_name].taint_source)
    ...
    def analyze_function(
        ...
        if not self.taint_sources:
            # 默认污点源：用户输入相关函数
            self.taint_sources = {'读取输入', '接收数据', 'zhc_read_int', 'zhc_read_float', 'zhc_read_char', 'zhc_read_string'}
