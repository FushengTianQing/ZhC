    ...
    def resolve(
        ...
        for candidate in candidates:
            ...
            if not candidate.type_info.param_types:
                ...
                if arg_types:
                    continue
                scored_candidates.append(OverloadCandidate(
                    symbol=candidate,
                    match_scores=[],
                    total_score=0,
                    is_exact_match=True,
        ...
        if not best:
            # 歧义：多个候选匹配度相同
            ambiguous = [c for c in scored_candidates if c.total_score == scored_candidates[0].total_score]
