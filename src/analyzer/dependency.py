...
class DependencyResolver:
    ...
    def parse_module_dependencies(self, module_declaration: Dict[str, Any]) -> ModuleInfo:
        """从模块声明中解析依赖关系"""
    ...
    def calculate_compilation_order(self) -> List[str]:
        ...
        while queue:
            ...
            for dependent in self.graph.get_dependents(current):
                ...
                if debug:
                    print(f"减少 {dependent} 的入度: {in_degree[dependent] + 1} -> {in_degree[dependent]}")
        ...
        return {
            "modules": {
                name: {
                    ...
                    "transitive_deps": list(self.graph.get_transitive_dependencies(name)),
    ...
    def get_module_statistics(self) -> Dict[str, Any]:
        ...
        total_dependencies = sum(len(module.dependencies) for module in self.graph.modules.values())
        total_public_symbols = sum(len(module.public_symbols) for module in self.graph.modules.values())
        total_private_symbols = sum(len(module.private_symbols) for module in self.graph.modules.values())
        ...
        return {
            ...
            "modules_without_deps": sum(1 for module in self.graph.modules.values() if not module.dependencies),
            "most_dependent_module": max(self.graph.modules.values(), 
                                        key=lambda m: len(m.dependencies)).name if self.graph.modules else None
        }
...
class MultiFileIntegrator:
    ...
    def register_module_file(self, module_name: str, file_path: str, content: str) -> None:
        """注册模块文件"""
        self.module_files[module_name] = file_path
        self.file_contents[file_path] = content
    
    def register_c_file(self, module_name: str, c_file_path: str, h_file_path: str) -> None:
        """注册C文件路径"""
...
"""
    ...
    def _generate_recommendations(self, stats: Dict, cycles: List, missing: List) -> List[str]:
        """生成优化建议"""
        recommendations = []
        ...
        if missing:
            recommendations.append(f"发现{len(missing)}个缺失的模块依赖，请检查模块定义")
