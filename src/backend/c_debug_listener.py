...
class CDebugListener:
    ...
    def on_variable(
        ...
        """
        self.generator.add_variable(
            name=name, type_name=type_name, line_number=line_number, address=address
        )
    ...
    def on_type_definition(
        ...
        if type_kind == "base":
            ...
            type_info.add_struct_type(
                type_name=type_name, members=members or [], byte_size=byte_size
            )
        elif type_kind == "pointer":
            # 指针类型
            base_type = members[0].get("base_type", "void") if members else "void"
            type_info.add_pointer_type(
                type_name=type_name, base_type=base_type, byte_size=byte_size
            )
    ...
    def on_reset(self) -> None:
        ...
        清空当前调试信息。
        """
        self.generator = CoreDebugInfoGenerator(self.source_file, self.output_file)
