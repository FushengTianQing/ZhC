...
class PrecompiledHeaderManager:
    ...
    def preprocess_header(self, filepath: str, content: str = None) -> PrecompiledHeader:
        """
        预处理头文件
        ...
        header_index.last_modified = path.stat().st_mtime if path.exists() else time.time()
    ...
    def _parse_macro(self, line: str, line_num: int, filepath: str) -> Optional[MacroDefinition]:
        """解析宏定义"""
        import re
    ...
    def _is_function_decl(self, line: str) -> bool:
        """检查是否是函数声明"""
        # 简化检查
        return '(' in line and ')' in line and ('函数' in line or 'function' in line.lower())
    
    def _parse_function_decl(self, line: str, line_num: int, filepath: str) -> Optional[HeaderSymbol]:
        """解析函数声明"""
        import re
    ...
    def _parse_type_decl(self, line: str, line_num: int, filepath: str) -> Optional[TypeDefinition]:
        """解析类型声明"""
        import re
        ...
        if match:
            kind = 'struct' if 'struct' in match.group(1).lower() or '结构体' in match.group(1) else 'class'
            name = match.group(2)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            'avg_parse_time': self.stats['total_parse_time'] / max(self.stats['total_headers'], 1),
    ...
    def generate_report(self) -> str:
        ...
        if self.headers:
            ...
            for filepath, pch in list(self.headers.items())[:10]:
                ...
                lines.append(f"  {Path(filepath).name}: {symbols_count} 符号 (命中{pch.cache_hits}次)")
