...
class CompilationCache:
    ...
    def _evict_if_needed(self, new_entry_size: int) -> None:
        ...
        while available_space < new_entry_size and entries:
            ...
            print(f"  🗑️  驱逐缓存: {key[:8]}... (命中: {entry.hits}, 大小: {entry.size} B)")
    ...
    def put(self, category: str, data: Any, *args, dependencies: Optional[List[str]] = None, metadata: Optional[Dict] = None) -> str:
        """
        保存数据到缓存
    ...
    def get_stats(self) -> Dict:
        ...
        if self.stats['total_hits'] + self.stats['total_misses'] > 0:
            hit_rate = self.stats['total_hits'] / (self.stats['total_hits'] + self.stats['total_misses']) * 100
        ...
        return {
            **self.stats,
            ...
            'space_used_percent': (self.stats['current_size'] / self.max_size) * 100 if self.max_size > 0 else 0
        }
...
class FileHashCache:
    ...
    def get_file_hash(self, filepath: Path) -> str:
        ...
        """
        # 首先检查缓存
        cached_hash = self.cache.get("file_hash", str(filepath), filepath.stat().st_mtime)
        if cached_hash:
            return cached_hash
