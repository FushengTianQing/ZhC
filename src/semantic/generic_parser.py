...
class GenericTypeNode(ASTNode):
    ...
    def __init__(self, base_type: str, type_args: List['TypeNode'] = None,
    ...
    def name(self) -> str:
        """获取完整类型名"""
        if self.type_args:
            args_str = ", ".join(arg.name if hasattr(arg, 'name') else str(arg) for arg in self.type_args)
...
class TypeNode(ASTNode):
    ...
    def __init__(self, type_name: str, is_generic: bool = False,
                 generic_args: List['TypeNode'] = None, line: int = 0, column: int = 0):
...
class TypeParameterNode(ASTNode):
    ...
    def __init__(self, name: str, variance: Variance = Variance.INVARIANT,
                 constraints: List[str] = None, default_type: Optional[str] = None,
                 line: int = 0, column: int = 0):
...
class WhereClauseNode(ASTNode):
    ...
    def __init__(self, constraints: List[Tuple[str, str]] = None, line: int = 0, column: int = 0):
        super().__init__(ASTNodeType.PARAM_DECL, line, column)
        self.constraints = constraints or []
...
class GenericTypeDeclNode(ASTNode):
    ...
    def __init__(self, name: str, type_params: List[TypeParameterNode] = None,
                 members: List[ASTNode] = None, where_clause: Optional[WhereClauseNode] = None,
                 line: int = 0, column: int = 0):
...
class GenericFunctionDeclNode(ASTNode):
    ...
    def __init__(self, name: str, type_params: List[TypeParameterNode] = None,
                 params: List[ASTNode] = None, return_type: Optional[TypeNode] = None,
                 body: Optional[ASTNode] = None, where_clause: Optional[WhereClauseNode] = None,
                 line: int = 0, column: int = 0):
...
class GenericParserMixin:
    ...
    def parse_type_reference(self) -> TypeNode:
        ...
        # 基本类型关键字
        if self.match(TokenType.INT, TokenType.FLOAT, TokenType.CHAR, TokenType.BOOL,
                      TokenType.VOID, TokenType.STRING, TokenType.DOUBLE, TokenType.LONG,
    ...
    def parse_type_parameter_declaration(self) -> TypeParameterNode:
        ...
        if self.match(TokenType.COLON):
            self.advance()  # 消耗 :
            # 解析约束列表
            while not self.check(TokenType.COMMA) and not self.check(TokenType.GT) \
                  and not self.check(TokenType.EQ) and not self.is_at_end():
    ...
    def parse_where_clause(self) -> WhereClauseNode:
        ...
        while not self.check(TokenType.LBRACE) and not self.is_at_end():
            ...
            # 解析约束列表
            while not self.check(TokenType.COMMA) and not self.check(TokenType.LBRACE) \
