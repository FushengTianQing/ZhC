...
class AsyncFunctionType:
    ...
    def _types_compatible(self, t1: Optional[AsyncType], t2: Optional[AsyncType]) -> bool:
        """检查两个类型是否兼容"""
...
class AsyncSemanticAnalyzer:
    ...
    def analyze_async_function(self, node: AsyncFunctionNode) -> bool:
        ...
        if node.return_type is None:
            self.warnings.append(f"异步函数 {node.name} 缺少返回类型标注 (行 {node.line})")
        else:
            # 检查返回类型是否是异步类型
        ...
        for param in node.parameters:
            if param.param_type is None:
                self.warnings.append(
                    f"参数 {param.name} 缺少类型标注 (行 {node.line})"
                )
    ...
    def _analyze_await(self, node: AwaitNode):
        ...
        if expr_type is None:
            self.warnings.append(
                f"无法推导 await 表达式的类型 (行 {node.line})"
            )
            return
    ...
    def _analyze_return(self, node: ReturnNode):
        ...
        if node.value is not None:
            ...
            if self._is_awaitable_type(expected_type):
                ...
                if expected_inner_type and not self._type_matches(value_type, expected_inner_type):
                    self.errors.append(
        ...
        return {
            ...
            "async_functions": {
                name: {
                    ...
                    "return_type": str(func_type.return_type) if func_type.return_type else None
                }
...
if __name__ == "__main__":
    ...
    func_node = AsyncFunctionNode(
        name="异步读取",
        parameters=[
            Parameter("path", AsyncType("字符串"))
        ],
        return_type=create_future_type(AsyncType("字符串")),
        body=[
            AwaitNode(expression=IdentifierExpr(name="some_async_call"))
        ]
    )
