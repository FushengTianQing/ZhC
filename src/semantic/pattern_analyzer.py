...
class PatternSemanticAnalyzer:
    ...
    def _check_pattern_type(
        ...
        if isinstance(pattern, WildcardPattern):
            ...
            elif expected_type.name not in ("整数型", "浮点型", "双精度浮点型", "长整数型", "短整数型"):
                result.success = False
                result.errors.append(
                    f"范围模式只能用于数值类型，不能用于 {expected_type.name}"
                )
            ...
            if not expected_type.is_struct():
                ...
                for field_name, field_pattern in pattern.fields.items():
                    if field_name not in expected_type.fields:
                        ...
                        field_result = self._check_pattern_type(field_pattern, field_type)
            ...
            if not expected_type.is_tuple():
                ...
                # 递归检查元素模式
                for elem_pattern, elem_type in zip(pattern.patterns, expected_type.tuple_types):
    ...
    def _is_literal_type_compatible(
        ...
        if literal.literal_type in ("INT", "FLOAT"):
            return type_info.name in ("整数型", "浮点型", "双精度浮点型", "长整数型", "短整数型")
    ...
    def _check_binding_consistency(
        ...
        result.bindings = {var: PatternTypeInfo("unknown", PatternTypeKind.UNKNOWN) 
    ...
    def _check_exhaustiveness(
        ...
        has_wildcard = any(
            ...
            isinstance(case.pattern, VariablePattern)
            for case in cases
        )
    ...
    def _is_pattern_covered(self, pattern: Pattern, covering: Pattern) -> bool:
        ...
        # 构造器模式：相同构造器覆盖
        if isinstance(pattern, ConstructorPattern) and isinstance(covering, ConstructorPattern):
        ...
        if isinstance(covering, OrPattern):
            return any(
                self._is_pattern_covered(pattern, sub)
                for sub in covering.patterns
            )
...
def create_struct_type(
    name: str,
    fields: Dict[str, PatternTypeInfo]
) -> PatternTypeInfo:
    """创建结构体类型信息"""
    return PatternTypeInfo(
        name=name,
        kind=PatternTypeKind.STRUCT,
        fields=fields
    )
...
def create_primitive_type(name: str) -> PatternTypeInfo:
    ...
    return PatternTypeInfo(
        name=name,
        kind=PatternTypeKind.PRIMITIVE
    )