...
if TYPE_CHECKING:
    from .type_checker import TypeInfo, TypeChecker
...
class InstantiationContext:
    ...
    def push_type(self, type_name: str) -> bool:
        ...
        if type_name in self.instantiation_stack:
            self.errors.append(f"检测到循环依赖: {' -> '.join(self.instantiation_stack)} -> {type_name}")
            return False
...
class GenericInstantiator:
    ...
    def instantiate_function(
        ...
        # 6. 实例化返回类型
        instantiated_return = self._substitute_type(generic_func.return_type, type_mapping)
        
        # 7. 实例化函数体（如果需要）
        instantiated_body = None
        if generic_func.body:
            instantiated_body = self._instantiate_body(generic_func.body, type_mapping, context)
    ...
    def _instantiate_members(
        ...
        for member in members:
            if isinstance(member, MemberInfo):
                ...
                instantiated.append(MemberInfo(
                    name=member.name,
                    type_name=new_type,
...
if __name__ == "__main__":
    ...
    generic_list = create_generic_type(
        name="列表",
