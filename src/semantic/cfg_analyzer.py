...
from ..parser.ast_nodes import (
    ...
    VariableDeclNode, AssignExprNode, IdentifierExprNode,
)
...
def ast_stmt_to_dict(node: ASTNode) -> dict:
    ...
    if nt == ASTNodeType.IF_STMT:
        result['type'] = 'if'
        result['then_body'] = ast_to_statements(node.then_branch) if node.then_branch else []
        result['else_body'] = ast_to_statements(node.else_branch) if node.else_branch else []
        # 提取条件表达式
...
def _expr_to_str(node: ASTNode) -> str:
    ...
    if nt == ASTNodeType.IDENTIFIER_EXPR:
        ...
        target_type = _type_to_str(node.target_type) if hasattr(node, 'target_type') else ''
        operand = _expr_to_str(node.operand) if hasattr(node, 'operand') else ''
...
class UninitAnalyzer:
    ...
    def _scan_block(self, node: ASTNode, initialized: Set[str],
                    func_name: str, symbol_table) -> None:
        """扫描代码块，跟踪变量初始化状态"""
        if node is None:
            return
    ...
    def _scan_statement(self, node: ASTNode, initialized: Set[str],
                        func_name: str, symbol_table) -> bool:
        """扫描单个语句
        ...
        if nt == ASTNodeType.VARIABLE_DECL:
            ...
            if hasattr(node, 'stmt') and node.stmt:
                return self._scan_statement(node.stmt, initialized, func_name, symbol_table)
            return False
    ...
    def _scan_expr(self, node: ASTNode, initialized: Set[str],
                   func_name: str, symbol_table) -> None:
        """扫描表达式，检测未初始化变量使用"""
        if node is None:
            return
        ...
        elif nt in (ASTNodeType.TERNARY_EXPR, ASTNodeType.CAST_EXPR,
