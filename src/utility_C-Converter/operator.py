...
class OperatorOverloadParser:
    ...
    def __init__(self):
        self.overloads: Dict[
            str, List[OperatorOverloadInfo]
        ] = {}  # class_name -> [overloads]
    ...
    def parse_operator_declaration(
        self, line: str, line_num: int
    ) -> Optional[OperatorOverloadInfo]:
        """解析运算符重载声明"""
        ...
        overload = OperatorOverloadInfo(
            self.current_class, operator, method_name, param_type
        )
...
class OperatorOverloadGenerator:
    ...
    def register_overload(
        self, class_name: str, operator: OperatorType, param_type: Optional[str] = None
    ):
        """注册运算符重载"""
        ...
        method_name = op_info[0]
        overload = OperatorOverloadInfo(
            class_name, operator, method_name, param_type or class_name
        )
    ...
    def generate_header(self, class_name: str) -> str:
        ...
        lines.extend(["", f"#endif /* {class_name.upper()}_OPERATOR_OVERLOAD_H */"])
        return "\n".join(lines)

    def generate_implementation(
        self, class_name: str, implementations: Dict[str, str]
    ) -> str:
        """生成运算符重载实现"""
        overloads = self.parser.get_class_overloads(class_name)
        if not overloads:
