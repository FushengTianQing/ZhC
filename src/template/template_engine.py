...
class TemplateEngine:
    ...
    def parse(self, template: str) -> List[TemplateBlock]:
        ...
        while pos < length:
            ...
            # 找到最近的标记
            next_positions = [
                p for p in [next_var, next_block, next_expr, next_comment]
                if p is not None
            ]
    ...
    def _parse_block_tag(self, template: str, pos: int) -> Tuple[Optional[TemplateBlock], int]:
        """
        解析块标记
        ...
        if block_type == 'if':
            ...
            if len(parts) >= 3 and parts[1] == 'in':
                variable = parts[0]
                collection = parts[2]
                children, new_pos = self._parse_block_children(template, new_pos, 'endfor')
    ...
    def _parse_block_children(self, template: str, pos: int, end_marker: str) -> Tuple[List[TemplateBlock], int]:
        """
        解析块的子内容
