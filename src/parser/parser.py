...
class ErrorRecovery:
    ...
    def is_balanced(self) -> bool:
        """检查嵌套结构是否平衡"""
        return self.brace_depth == 0 and self.paren_depth == 0 and self.bracket_depth == 0
...
class Parser(GenericParserMixin):
    ...
    def synchronize(self):
        ...
        while not self.is_at_end():
            ...
            if token.type in (
                ...
                TokenType.DO, TokenType.SWITCH, TokenType.GOTO
            ):
                return
    ...
    def parse_module_decl(self) -> ModuleDeclNode:
        ...
        return ModuleDeclNode(name, exports, imports, body,
                             self.tokens[0].line, self.tokens[0].column)
    ...
    def parse_import_decl(self) -> ImportDeclNode:
        ...
        return ImportDeclNode(module_name, symbols,
    ...
    def parse_union_decl(self) -> UnionDeclNode:
        ...
        return UnionDeclNode(name, members,
                             self.tokens[self.pos-1].line, self.tokens[self.pos-1].column)
    ...
    def parse_enum_decl(self) -> EnumDeclNode:
        ...
        return EnumDeclNode(name, values,
                            self.tokens[self.pos-1].line, self.tokens[self.pos-1].column)
    ...
    def parse_typedef_decl(self) -> TypedefDeclNode:
        ...
        return TypedefDeclNode(old_type, new_name,
                               self.tokens[self.pos-1].line, self.tokens[self.pos-1].column)
    ...
    def parse_variable_decl(self) -> VariableDeclNode:
        ...
        if fp_result is not None:
            ...
            return VariableDeclNode(name, var_type, init, False,
                                   self.tokens[self.pos-1].line, self.tokens[self.pos-1].column)
    ...
    def parse_const_decl(self) -> VariableDeclNode:
        ...
        return VariableDeclNode(name, var_type, init, True,
                               self.tokens[self.pos-1].line, self.tokens[self.pos-1].column)
    ...
    def parse_param_decl(self) -> ParamDeclNode:
        ...
        return ParamDeclNode(name, param_type, default_value,
                            self.tokens[self.pos-1].line, self.tokens[self.pos-1].column)
    ...
    def parse_statement(self) -> Optional[ASTNode]:
        ...
        if (self.match(TokenType.IDENTIFIER) and 
            not self.is_at_end() and self.peek_token().type == TokenType.COLON):
            label_token = self.current_token()
            self.advance()  # 消耗标识符
            self.advance()  # 消耗 ':'
            stmt = self.parse_statement()
            return LabelStmtNode(label_token.value, stmt,
    ...
    def parse_break_stmt(self) -> BreakStmtNode:
        ...
        return BreakStmtNode(self.tokens[self.pos-1].line, self.tokens[self.pos-1].column)
    ...
    def parse_continue_stmt(self) -> ContinueStmtNode:
        ...
        return ContinueStmtNode(self.tokens[self.pos-1].line, self.tokens[self.pos-1].column)
    ...
    def parse_do_while_stmt(self) -> DoWhileStmtNode:
        ...
        return DoWhileStmtNode(body, condition,
                              do_token.line, do_token.column)
    ...
    def parse_switch_stmt(self) -> SwitchStmtNode:
        ...
        return SwitchStmtNode(expr, cases,
    ...
    def parse_case_stmt(self) -> CaseStmtNode:
        ...
        while (not self.is_at_end() and 
               not self.match(TokenType.CASE) and 
               not self.match(TokenType.DEFAULT) and
               not self.match(TokenType.RBRACE)):
            stmt = self.parse_statement()
            if stmt:
                statements.append(stmt)
        
        return CaseStmtNode(value, statements,
    ...
    def parse_default_stmt(self) -> DefaultStmtNode:
        ...
        return DefaultStmtNode(statements,
    ...
    def parse_goto_stmt(self) -> GotoStmtNode:
        ...
        return GotoStmtNode(label_token.value,
                          goto_token.line, goto_token.column)
    ...
    def parse_expr_stmt(self) -> ExprStmtNode:
        ...
        return ExprStmtNode(expr, self.tokens[self.pos-1].line, self.tokens[self.pos-1].column)
    ...
    def try_parse_function_pointer(self, base_type: ASTNode) -> Optional[Tuple[str, ASTNode]]:
    ...
    def parse_unary(self) -> ASTNode:
        ...
        if self.match(TokenType.MINUS, TokenType.NOT, TokenType.BIT_NOT,
                     ...
                     TokenType.BIT_AND, TokenType.STAR):
            operator = self.advance().value
            operand = self.parse_unary()
            return UnaryExprNode(operator, operand, True,
                                self.tokens[self.pos-1].line, self.tokens[self.pos-1].column)
    ...
    def parse_primary(self) -> ASTNode:
        ...
        if self.match(TokenType.TRUE, TokenType.FALSE):
            self.advance()
            return BoolLiteralNode(token.type == TokenType.TRUE, token.line, token.column)
