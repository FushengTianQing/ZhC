...
class MethodConverter:
    ...
    def convert_method(
        ...
        # 转换方法名
        converted_name = self._convert_method_name(
            class_name, method_name, is_constructor, is_destructor
        )
    ...
    def _parse_parameters(self, params_str: str) -> List[ParameterInfo]:
        ...
        for type_name, param_name in matches:
            ...
            param = ParameterInfo(name=param_name, type_name=type_name, c_type=c_type)
            params.append(param)

        return params

    def _convert_method_name(
        self,
        class_name: str,
        method_name: str,
        is_constructor: bool,
    ...
    def _convert_method_body(
        self, method_code: str, class_name: str, params: List[ParameterInfo]
    ) -> str:
        """转换方法体"""
...
class VirtualMethodTableGenerator:
    ...
    def create_virtual_table(
        ...
        class_name: str,
        methods: List[str],
        base_tables: Optional[List[Any]] = None,
    ) -> VirtualTable:
        """创建虚函数表"""
        vtable = VirtualTable(class_name=class_name, base_tables=base_tables or [])

        for method in methods:
            entry = VirtualTableEntry(
                method_name=method, method_signature=f"void ({class_name}*)(void)"
            )
...
if __name__ == "__main__":
    ...
    result3 = converter.convert_method(
        class_name="学生", method_code="函数 获取版本() -> 整数型", is_static=True
    )
    ...
    vtable = vtable_gen.create_virtual_table("学生", ["获取信息", "设置成绩"])
