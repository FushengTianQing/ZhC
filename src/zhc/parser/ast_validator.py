...
class ValidationIssue:
    ...
    def __str__(self) -> str:
        ...
        result = (
            f"[{severity_str}] 第 {self.line} 行，第 {self.column} 列: {self.message}"
        )
        if self.suggestion:
            result += f"\n  建议: {self.suggestion}"
        return result
...
class ValidationResult:
    ...
    def add_info(self, message: str, node: ASTNode):
        ...
        self.issues.append(
            ValidationIssue(
                severity=ValidationSeverity.INFO, message=message, node=node
            )
        )
...
class ASTValidator:
    ...
    def _validate_node(self, node: ASTNode, result: ValidationResult):
        ...
        if self.current_depth > self.max_depth:
            result.add_error(
                f"AST 深度超过限制 ({self.max_depth})", node, "检查是否存在无限递归"
            )
            self.current_depth -= 1
            return
    ...
    def _validate_function_decl(self, node: FunctionDeclNode, result: ValidationResult):
        ...
        if node.body:
            ...
            result.add_warning(f"函数 '{node.name}' 没有函数体", node, "添加函数体实现")
    ...
    def _validate_variable_decl(self, node: VariableDeclNode, result: ValidationResult):
        ...
        if node.name in self.declared_symbols:
            result.add_error(
                f"变量 '{node.name}' 重复声明", node, "修改变量名或删除重复声明"
            )
    ...
    def _validate_return_stmt(self, node: ReturnStmtNode, result: ValidationResult):
        ...
        if not self.in_function:
            result.add_error("return 语句不在函数内", node, "将 return 语句移到函数内")
            return

        # 获取返回类型名称
        return_type = (
            self.current_function.return_type if self.current_function else None
        )
        ...
        if node.value:
            ...
            if return_type_name == "空":
                result.add_warning(
                    "函数返回类型为 '空' 但有返回值", node, "删除返回值或修改返回类型"
                )
        else:
            # 检查是否需要返回值
            if return_type_name and return_type_name != "空":
    ...
    def _validate_call_expr(self, node: CallExprNode, result: ValidationResult):
        ...
        if isinstance(node.callee, IdentifierExprNode):
            ...
            if func_name not in self.function_symbols:
                result.add_warning(f"函数 '{func_name}' 未声明", node, "确保函数已声明")
            else:
                # 检查参数数量
    ...
    def _validate_identifier_expr(
        self, node: IdentifierExprNode, result: ValidationResult
    ):
        """验证标识符表达式"""
        # 检查变量是否声明
        if (
            node.name not in self.declared_symbols
            and node.name not in self.function_symbols
        ):
            result.add_warning(f"变量 '{node.name}' 未声明", node, "确保变量已声明")
...
if __name__ == "__main__":
    ...
    # 测试 2: 函数缺少返回语句
    print("\n测试 2: 函数缺少返回语句")
    func2 = FunctionDeclNode(
        name="测试函数",
        return_type="整数",
        params=[],
        body=BlockStmtNode(statements=[]),
    )
    program2 = ProgramNode(declarations=[func2])
    ...
    func3 = FunctionDeclNode(
        ...
        return_type="整数",
        params=[],
        body=BlockStmtNode(statements=[ReturnStmtNode(value=IntLiteralNode(value=42))]),
    )
