...
class CompilationPipeline:
    ...
    def _generate_markdown_report(
        self,
        processed_files: List[Optional[Dict]],
...
"""
    ...
    def process_file(
        self, filepath: Path, is_main: bool = False
                    ...
                    module_declarations.append(
                        {
                            ...
                            "imports": [
                                getattr(imp, "module_name", "")
                                for imp in getattr(decl, "imports", [])
                            ],
    ...
    def compile_project(
        self, source_files: List[str], output_name: str = "a.out"
    ) -> bool:
        """
        编译整个项目
        ...
        for filepath in source_paths:
            if not filepath.exists():
                self.error_handler.add_error(
                    ErrorType.FILE_NOT_FOUND, f"文件不存在: {filepath}", line_no=0
                )
                continue
            ...
            if test_mode:
                ...
                try:
                    ...
                    compile_result = subprocess.run(
                        ["make", "-j4"], capture_output=True, text=True, check=False
                    )
        ...
        logger.info(
            "性能统计: 总文件=%d, 缓存命中=%d (%.1f%%), 解析=%d, 转换=%d, 依赖分析=%d, 总用时=%.3fs",
    ...
    def incremental_compile(
        ...
        # 重新计算编译顺序并编译
        return self.compile_project([str(f) for f in source_paths], output_name)
...
def main():
    ...
    # 创建编译流水线
    pipeline = CompilationPipeline(enable_cache=not args.no_cache)
