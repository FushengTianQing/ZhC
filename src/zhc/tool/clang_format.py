    ...
    def _check_availability(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [self.clang_format_path, "--version"], capture_output=True, timeout=5
            )
            return result.returncode == 0
        except (subprocess.TimeoutExpired, FileNotFoundError):
            return False

    def format_code(self, code: str, filename: Optional[str] = None) -> FormatResult:
        """
        格式化代码
        ...
        if self.available:
            # 使用clang-format
            try:
                args = [self.clang_format_path, f"--style={self.style}"]
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            **self.stats,
            ...
            "success_rate": (
                self.stats["successful_formats"] / max(1, self.stats["total_formats"])
            ),
        }
