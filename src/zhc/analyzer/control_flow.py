...
class ControlFlowGraph:
    ...
    def create_node(
        self, node_type: NodeType, basic_block: Optional[BasicBlock] = None
    ) -> str:
        """创建节点"""
        node_id = f"n{self._node_counter}"
        self._node_counter += 1

        node = CFGNode(id=node_id, node_type=node_type, basic_block=basic_block)
...
class ControlFlowAnalyzer:
    ...
    def build_cfg(self, func_name: str, statements: List[dict]) -> ControlFlowGraph:
        ...
        # 处理语句
        current_block = self._process_statements(
            cfg, statements, current_block, cfg.exit_id
        )

        # 如果当前块不是出口，连接到出口
        if current_block and current_block.id in cfg.nodes:
            if not any(
                sid == cfg.exit_id for sid in cfg.get_successors(current_block.id)
            ):
    ...
    def _process_if(
        self, cfg: ControlFlowGraph, stmt: dict, current_block: BasicBlock, exit_id: str
    ) -> BasicBlock:
        """处理if语句"""
        # 当前块结束，创建分支节点
        ...
        if "else_body" in stmt and stmt["else_body"]:
            ...
            else_block = self._process_statements(
                cfg, stmt["else_body"], else_block, exit_id
            )
    ...
    def _process_break(
        self, cfg: ControlFlowGraph, stmt: dict, current_block: BasicBlock
    ):
        """处理break语句"""
        current_block.add_statement(stmt)
        # 实际实现中需要找到最近的循环退出点
            ...
            if not has_exit:
                ...
                issues.append(
                    FlowIssue(
                        ...
                        message="检测到可能的无限循环",
                        line_number=loop_header.basic_block.start_line
                        if loop_header.basic_block
                        else 0,
                        severity="warning",
...
if __name__ == "__main__":
    ...
    test_statements = [
        ...
        {"type": "var_decl", "name": "y", "line": 2},
        {
            "type": "if",
            "condition": "x > 0",
            "then_body": [{"type": "assign", "name": "y", "line": 4}],
            "else_body": [{"type": "assign", "name": "y", "line": 6}],
            "line": 3,
        },
