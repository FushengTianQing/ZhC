...
class CallGraph:
    ...
    call_sites: Dict[str, List[CallSite]] = field(
...
class InterproceduralAnalyzer:
    ...
    def analyze_side_effects(
        ...
        # 判断是否纯函数
        summary.is_pure = (
            len(summary.side_effects) == 0 and len(summary.modifies_globals) == 0
        )
    ...
    def _analyze_side_effects_in_statements(
        self, func_name: str, statements: List[dict], summary: FunctionSummary
    ):
        """分析语句中的副作用"""
        for stmt in statements:
            stmt_type = stmt.get("type", "")
            ...
            if stmt_type in ("dereference", "pointer_write"):
                summary.side_effects.add(
                    SideEffect.READ_MEMORY
                    if "read" in stmt_type
                    else SideEffect.WRITE_MEMORY
                )
            ...
            # 递归分析
            if "body" in stmt:
                self._analyze_side_effects_in_statements(
                    func_name, stmt["body"], summary
                )

    # ==================== 过程间常量传播 ====================

    def propagate_constants_interprocedurally(
        self, func_name: str, constants: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        过程间常量传播
    ...
    def analyze_with_context(
        self, func_name: str, context: str, known_types: Dict[str, str]
    ) -> Dict[str, str]:
        """
        上下文敏感分析
        ...
        for i, param in enumerate(summary.parameters):
            param_type = (
                summary.parameter_types[i] if i < len(summary.parameter_types) else None
            )
            if param_type:
    ...
    def generate_report(self) -> str:
        ...
        lines.append(
            f"  调用边数：{sum(len(v) for v in self.call_graph.nodes.values())}"
        )
        lines.append("")
