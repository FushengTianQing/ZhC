...
class TreeEditDistance:
    ...
    def compute_distance(
        self, old_node: Optional[ASTNode], new_node: Optional[ASTNode]
    ) -> int:
        """计算树编辑距离"""
    ...
    def _compute_recursive(self, old_node: ASTNode, new_node: ASTNode) -> int:
        ...
        cost = self._compute_child_distance(
            old_node.get_children(), new_node.get_children()
        )
    ...
    def _compute_child_distance(
        self, old_children: List[ASTNode], new_children: List[ASTNode]
    ) -> int:
        """计算子节点列表的距离（动态规划）"""
        if not old_children:
    ...
    def _compute_edit_script_recursive(
        self, old_node: Optional[ASTNode], new_node: Optional[ASTNode]
    ):
        """递归生成编辑脚本"""
        if old_node is None and new_node is None:
    ...
    def _match_children(self, old_children: List[ASTNode], new_children: List[ASTNode]):
        """匹配子节点"""
        n = len(old_children)
        m = len(new_children)
        ...
        for i in range(n):
            for j in range(m):
                if dist[i][j] == 0 and i not in used_old and j not in used_new:
                    self._compute_edit_script_recursive(
                        old_children[i], new_children[j]
                    )
                    used_old.add(i)
                    used_new.add(j)
...
class IncrementalASTUpdater:
    ...
    def _compute_children_diff(
        self, old_children: List[ASTNode], new_children: List[ASTNode], parent_id: str
    ):
        """计算子节点的差异"""
        ...
        # 删除的节点
        deleted_children = [
            child for child in old_children if child.get_hash() not in new_hashes
        ]
