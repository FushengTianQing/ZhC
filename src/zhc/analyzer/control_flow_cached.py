...
# 导入基础控制流分析器
from .control_flow import ControlFlowAnalyzer, ControlFlowGraph, FlowIssue
...
class ControlFlowAnalyzerCached(ControlFlowAnalyzer):
    ...
    def _put_cfg_to_cache(self, func_name: str, cfg: ControlFlowGraph, source: str):
        """将CFG存入缓存"""
        # LRU淘汰
    ...
    def build_cfg_cached(
        self, func_name: str, statements: List[dict], source: str = ""
    ) -> ControlFlowGraph:
        """
        带缓存的CFG构建
    ...
    def detect_unreachable_code_cached(self, cfg: ControlFlowGraph) -> List[FlowIssue]:
        """
        带缓存的不可达代码检测
        ...
        self._unreachable_cache[cache_key] = AnalysisCacheEntry(
            result=result, cfg_hash=cfg_hash, compute_time=compute_time, access_count=1
        )

        return result

    def compute_cyclomatic_complexity_cached(self, cfg: ControlFlowGraph) -> int:
        """
        带缓存的圈复杂度计算
    ...
    def detect_infinite_loops_cached(self, cfg: ControlFlowGraph) -> List[FlowIssue]:
        """
        带缓存的无限循环检测
