...
class LineNumberTable:
    ...
    def get_line_for_address(self, address: int) -> Optional[Tuple[int, str]]:
        ...
        while low < high:
            ...
            if entry["address"] <= address:
                if (
                    mid == len(self.line_entries) - 1
                    or self.line_entries[mid + 1]["address"] > address
                ):
                    # 找到对应的条目
                    file_path = (
                        self.file_table[entry["file"]]
                        if entry["file"] < len(self.file_table)
                        else ""
                    )
    ...
    def to_json(self) -> Dict:
        """导出为JSON格式"""
        return {"file_table": self.file_table, "line_entries": self.line_entries}
...
class DebugSymbolTable:
    ...
    def add_label(
        self, name: str, address: int, file_path: str = "", line_number: int = 0
    ) -> None:
        """
        添加标签符号
    ...
    def lookup_symbol(
        self, name: str, scope_id: Optional[int] = None
    ) -> Optional[Dict]:
        """
        查找符号
...
class TypeInfoGenerator:
    ...
    def add_pointer_type(
        self, type_name: str, base_type: str, byte_size: int = 8
    ) -> int:
        """
        添加指针类型
        ...
        return self.add_type(
            type_name, "DW_TAG_pointer_type", byte_size, base_type=base_type
        )

    def add_array_type(
        self, type_name: str, element_type: str, array_size: int, element_size: int
    ) -> int:
        """
        添加数组类型
    ...
    def add_struct_type(
        self, type_name: str, members: List[Dict], byte_size: int
    ) -> int:
        """
        添加结构体类型
        ...
        """
        return self.add_type(
            type_name, "DW_TAG_structure_type", byte_size, members=members
        )

    def add_function_type(
        self, return_type: str, parameters: List[str], type_name: str = ""
    ) -> int:
        """
        添加函数类型
    ...
    def to_json(self) -> Dict:
        """导出为JSON格式"""
        return {"type_table": self.type_table}
...
class DWARFGenerator:
    ...
    def add_compile_unit(
        self, name: str, source_file: str, comp_dir: str = ""
    ) -> CompileUnit:
        """
        添加编译单元
        ...
        Returns:
            编译单元对象
        """
        unit = CompileUnit(name=name, comp_dir=comp_dir)
        self.compile_units.append(unit)

        # 添加源文件到行号表
        self.line_table.add_file(source_file)
...
class DebugInfoGenerator:
    ...
    def __init__(self, source_file: str, output_file: str = ""):
        ...
        self.unit = self.dwarf.add_compile_unit(
            name=Path(source_file).stem, source_file=source_file
        )
    ...
    def add_variable(
        self, name: str, type_name: str, line_number: int, address: int
    ) -> None:
        """
        添加变量调试信息
...
if __name__ == "__main__":
    ...
    # 添加变量
    debug_gen.add_variable(name="x", type_name="整数型", line_number=2, address=0x2000)
