        ...
        if existing:
            ...
            if (
                ...
                if existing_sig != new_sig:
                    ...
                    self.all_symbols[
                        f"{self.current_scope.scope_name}.{symbol.name}_{len(existing._overloads)}"
                    ] = symbol
                    return True
                # 参数签名相同，不允许
                return False
...
class SemanticAnalyzer:
    ...
    def _analyze_struct_decl(self, node: StructDeclNode) -> None:
        ...
        if not self.symbol_table.add_symbol(struct_symbol):
            self._add_error("重复定义", f"结构体 '{node.name}' 重复定义", loc)
        ...
        for member in node.members:
            ...
            if hasattr(member, "name"):
                ...
                struct_symbol.members.append(
                    Symbol(
                        name=member.name, symbol_type="成员变量", data_type=member_type
                    )
                )
    ...
    def _analyze_variable_decl(self, node: VariableDeclNode) -> None:
        ...
        if node.init:
            ...
            var_type_info = (
                ast_type_to_typeinfo(node.var_type) if node.var_type else None
            )
            init_type_info = self._infer_expr_type(node.init)
            if var_type_info and init_type_info:
                self._check_type_compat(
                    var_type_info, init_type_info, node, f"初始化变量 '{node.name}'"
                )
    ...
    def _analyze_break_stmt(self, node: BreakStmtNode) -> None:
        ...
        if not self.in_loop and not self.in_switch:
            self._add_error(
                "非法跳出", "跳出语句不在循环或选择语句中", self._node_location(node)
            )

    def _analyze_continue_stmt(self, node: ContinueStmtNode) -> None:
        """分析继续语句"""
        if not self.in_loop:
            self._add_error("非法继续", "继续语句不在循环中", self._node_location(node))
            ...
            if not member_found:
                ...
                if available:
                    ...
                    if not suggestions and len(available) <= 5:
                        ...
                        suggestions.append(
                            f"结构体 '{obj_type_name}' 有 {len(available)} 个成员"
                        )
        ...
        if actual_count != expected_count:
            if actual_count < expected_count:
                self._add_error(
                    ...
                    self._node_location(node),
                    suggestions=["补充缺少的参数", f"检查函数 '{func_name}' 的声明"],
                )
            else:
                self._add_error(
                    "参数过多",
                    f"函数 '{func_name}' 期望 {expected_count} 个参数，但提供了 {actual_count} 个",
                    self._node_location(node),
                    suggestions=["移除多余的参数", f"检查函数 '{func_name}' 的声明"],
                )
            return

        # T2.1: 参数类型检查（逐参数）
        for i, (declared_param, actual_arg) in enumerate(
            zip(declared_params, actual_args)
        ):
    ...
    def _resolve_overload(
        self, node, func_name: str, candidates: List[Symbol], actual_args: list
    ) -> None:
        """解析函数重载（Phase 6 T2.2）
        ...
        for candidate in candidates:
            ...
            score = 0
            match_ok = True
            for i, (declared, actual_arg) in enumerate(
                zip(declared_params, actual_args)
            ):
        ...
        for i, (declared_param, actual_arg) in enumerate(
            zip(best_candidate.parameters, actual_args)
        ):
            declared_type_info = self.type_checker.get_type(declared_param.data_type)
            actual_type_info = self._infer_expr_type(actual_arg)
        ...
        if nt == ASTNodeType.INT_LITERAL:
            ...
            if hasattr(value_node, "operand") and value_node.operand:
                ...
                if inner is not None:
                    op = getattr(value_node, "operator", "") or getattr(
                        value_node, "op", ""
                    )
                    return f"{op}{inner}"
            return None
    ...
    def _analyze_union_decl(self, node: UnionDeclNode) -> None:
        ...
        if not self.symbol_table.add_symbol(union_symbol):
            self._add_error(
                "重复定义", f"共用体 '{node.name}' 重复定义", self._node_location(node)
            )
            return
        self.stats["symbols_added"] += 1
    ...
    def _register_generic_type(self, node: StructDeclNode) -> None:
        ...
        try:
            ...
            generic_type = GenericType(
                name=node.name, type_params=type_params, members=member_types
            )
    ...
    def _infer_expr_type(self, node: ASTNode):
        ...
        # 标识符：查符号表
            ...
            symbol = (
                self.symbol_table.lookup(node.name) if hasattr(node, "name") else None
            )
        ...
        # 二元表达式
            ...
            left_type = (
                self._infer_expr_type(node.left) if hasattr(node, "left") else None
            )
            right_type = (
                self._infer_expr_type(node.right) if hasattr(node, "right") else None
            )
            op = (
                node.operator
                if hasattr(node, "operator")
                else (node.op if hasattr(node, "op") else None)
            )
        ...
        # 一元表达式
            ...
            operand_type = (
                self._infer_expr_type(node.operand)
            ...
            if operand_type and op:
                self.type_checker.clear()
                result = self.type_checker.check_unary_op(node.line, op, operand_type)
                for err_line, err_type, err_msg in self.type_checker.get_errors():
                    self._add_error(err_type, err_msg, self._node_location(node))
                self.type_checker.clear()
                return result
        ...
        # 函数调用
            ...
            if func_name:
                ...
                if self.generic_manager.is_generic_function(func_name):
                    ...
                    generic_funcs = self.generic_manager.get_generic_functions(
                        func_name
                    )
                    if generic_funcs:
                        type_args = inferrer.infer_type_arguments(
                            generic_funcs[0], arg_types
                        )
                        ...
                        if type_args:
                            ...
                            return self.type_checker.get_type(
                                instance.specialized_return_type
                            )
                # ===== Phase 9 结束 =====
        ...
        # 成员访问：查找结构体符号，返回成员类型
            ...
            if hasattr(node, "obj") and hasattr(node, "member"):
                ...
                if obj_type_name:
                    struct_symbol = self.symbol_table.lookup(obj_type_name)
                    if struct_symbol and struct_symbol.symbol_type in (
                        "结构体",
                        "共用体",
                    ):
                        for m in struct_symbol.members:
        ...
        # 三元表达式：返回 then/else 的公共类型
            ...
            then_type = (
                self._infer_expr_type(node.then_expr)
                if hasattr(node, "then_expr")
                else None
            )
            else_type = (
                self._infer_expr_type(node.else_expr)
    ...
    def _infer_with_engine(self, node: ASTNode):
        ...
        """
        from ..typeinfer.engine import TypeEnv
    ...
    def _check_type_compat(
        self, target_type_info, value_type_info, node: ASTNode, context: str
    ) -> None:
        ...
        if not ok:
            ...
            for warn_line, warn_type, warn_msg in self.type_checker.get_warnings():
                self._add_warning("类型警告", warn_msg, self._node_location(node))
                ...
                try:
                    ...
                    for issue in result.get("issues", []):
                        ...
                        if issue_type in (
                            ...
                            self._add_warning("内存安全", message, loc)
                except Exception:
                    pass  # 单函数分析失败不阻断
            ...
            for func_decl in find_functions(ast):
                ...
                try:
                    result = analyzer.analyze_function(
                        func_decl.name, stmt_dicts, params
                    )
            ...
            for func_decl in find_functions(ast):
                func_dict = {
                    ...
                    "param_types": [
                        ...
                        if hasattr(p, "param_type") and p.param_type
                    ],
                    ...
                    try:
                        known_types = {
                            ...
                            for i in range(
                                min(
                                    len(func_def["params"]),
                                    len(func_def["param_types"]),
                                )
                            )
                        }
                        inferred = ipa.analyze_with_context(
                            func_name, "semantic_analysis", known_types
                        )
                        # 如果推导出的类型与声明不一致，报告警告
                        for param, inferred_type in inferred.items():
                            if (
                                param in known_types
                                and inferred_type != known_types[param]
                            ):
                                self._add_warning(
                                    "类型推导",
        ...
        try:
            ...
            for func_decl in find_functions(ast):
                ...
                try:
                    ...
                    for param in params:
                        param_type = (
                            self._get_type_name(param.param_type)
                            if hasattr(param, "param_type") and param.param_type
                            else ""
                        )
                        if param_type and (
                            "唯一指针" in param_type
                            or "unique_ptr" in param_type
                            or "共享指针" in param_type
                            or "shared_ptr" in param_type
                        ):
                            try:
                                analyzer.analyze_smart_pointer(
                                    param.name,
                                    param_type,
                        ...
                        if stmt_type == "expression":
                            ...
                            if any(op in value for op in ["+", "-"]):
                                ...
                                for ptr_name in analyzer.pointers:
                                    if ptr_name in value and (
                                        "+" in value or "-" in value
                                    ):
                                        analyzer.check_pointer_arithmetic(
                                            ptr_name, "arithmetic", line
                                        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """获取统计信息"""
        return {**self.stats, "symbol_table": self.symbol_table.get_statistics()}
    ...
    def _print_analyzer_reports(self) -> None:
        ...
        try:
            ...
            uninit_warnings = [
                w for w in self.warnings if w.error_type == "未初始化变量"
            ]
        ...
        try:
            cfg_warnings = [
                w
                for w in self.warnings
                if w.error_type in ("不可达代码", "无限循环", "圈复杂度")
            ]
            if cfg_warnings:
                print("\n📋 控制流分析:")
        ...
        try:
            ip_warnings = [
                w
                for w in self.warnings
                if w.error_type in ("递归调用", "副作用", "类型推导")
            ]
            if ip_warnings:
                print("\n📋 过程间分析:")
