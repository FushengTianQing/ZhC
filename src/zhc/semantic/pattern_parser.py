...
class PatternParser:
    ...
    def expect(self, token_type: TokenType) -> Token:
        ...
        if token is None:
            raise PatternParserError(
                f"期望 {token_type.name}，但到达文件末尾", line=0, column=0
            )
    ...
    def _parse_or_pattern(self) -> Pattern:
        ...
        while self.match(TokenType.BIT_OR):
            ...
            left = OrPattern(
                patterns=[left, right], line=op_token.line, column=op_token.column
            )
    ...
    def _parse_primary_pattern(self) -> Pattern:
        ...
        if (
            ...
            and self.peek_token()
            and self.peek_token().type == TokenType.DOTDOT
        ):
        ...
        raise PatternParserError(
            f"不支持的模式类型: {token.type.name}", line=token.line, column=token.column
        )
    ...
    def _parse_bool_literal(self) -> LiteralPattern:
        ...
        return LiteralPattern(
            value=value, literal_type="BOOL", line=token.line, column=token.column
        )
    ...
    def _parse_null_literal(self) -> LiteralPattern:
        ...
        token = self.advance()
        return LiteralPattern(
            value=None, literal_type="NULL", line=token.line, column=token.column
        )
    ...
    def _parse_range_pattern(self) -> RangePattern:
        ...
        if end_token is None or end_token.type != TokenType.INT_LITERAL:
            raise PatternParserError(
                "范围模式需要结束值", line=start_token.line, column=start_token.column
            )
    ...
    def _parse_range_pattern_from_start(
        self, start_token: Token, start_value: int
    ) -> RangePattern:
        """从已解析的起始值继续解析范围模式"""
        self.expect(TokenType.DOTDOT)
    ...
    def _parse_tuple_pattern(self) -> TuplePattern:
        ...
        if self.match(TokenType.RPAREN):
            self.advance()
            return TuplePattern(
                patterns=patterns, line=lparen.line, column=lparen.column
            )
        ...
        return TuplePattern(patterns=patterns, line=lparen.line, column=lparen.column)
    ...
    def _parse_identifier_pattern(self) -> Pattern:
        ...
        return VariablePattern(
            name=id_token.value, line=id_token.line, column=id_token.column
        )

    def _parse_constructor_pattern(
        self, constructor_token: Token
    ) -> ConstructorPattern:
        """解析构造器模式
        ...
        while True:
            ...
            if self.match(TokenType.COLON):
                ...
                field_pattern = VariablePattern(
                    name=field_name, line=field_token.line, column=field_token.column
                )
