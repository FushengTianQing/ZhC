...
class AllocationResult:
    ...
    success: bool
    allocations: Dict[int, Register] = field(
        default_factory=dict
    )  # 虚拟寄存器ID -> 物理寄存器
    spills: List[int] = field(default_factory=list)  # 溢出的虚拟寄存器ID列表
    loads: List[Tuple[int, int]] = field(
        default_factory=list
    )  # (指令位置, 虚拟寄存器ID)
    stores: List[Tuple[int, int]] = field(
        default_factory=list
    )  # (指令位置, 虚拟寄存器ID)
...
class TargetArchitecture:
    ...
    def get_registers(cls, kind: RegisterKind = RegisterKind.ANY) -> List[Register]:
        ...
        if (
            ...
            or kind == RegisterKind.POINTER
            or kind == RegisterKind.ANY
        ):
...
class LinearScanRegisterAllocator:
    ...
    def build_intervals(self, instructions: List[dict]) -> List[LiveInterval]:
        ...
        for reg_id, uses in live_map.items():
            ...
            vreg = VirtualRegister(
                id=reg_id, name=f"v{reg_id}", kind=RegisterKind.INTEGER
            )

            interval = LiveInterval(virtual_reg=vreg, start=start, end=end, uses=uses)
    ...
    def allocate(self, instructions: List[dict]) -> AllocationResult:
        ...
        for interval in intervals:
            if interval.virtual_reg.spilled:
                ...
                result.allocations[interval.virtual_reg.id] = (
                    interval.virtual_reg.assigned
                )
    ...
    def generate_spill_code(
        self, interval: LiveInterval, before_pos: int, after_pos: int
        ...
        for i, interval1 in enumerate(intervals):
            for interval2 in intervals[i + 1 :]:
                if interval1.overlaps(interval2):
                    self.add_conflict(
                        interval1.virtual_reg.id, interval2.virtual_reg.id
                    )
...
def simple_allocate(instructions: List[dict], num_regs: int = 8) -> AllocationResult:
    """
    简单的寄存器分配接口
