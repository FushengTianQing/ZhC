...
class LengauerTarjanDominator:
    ...
    def build(
        self, entry: str, blocks: Dict[str, Tuple[List[str], List[str]]]
    ) -> Dict[str, str]:
        """
        构建支配树
    ...
    def _compress(self, v: str):
        ...
        if (
            self.ancestor[v] in self.dfs_num
            and self.ancestor[self.ancestor[v]] in self.dfs_num
        ):
            if (
                self.dfs_num[self.semi[self.label[self.ancestor[v]]]]
                < self.dfs_num[self.semi[self.label[v]]]
            ):
...
def build_dominator_tree_iterative(
    entry: str, blocks: Dict[str, Tuple[List[str], List[str]]]
