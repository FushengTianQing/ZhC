...
class SanitizerIntegration:
    ...
    def _check_availability(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [self.clang_path, "--version"], capture_output=True, timeout=5
            )
            return result.returncode == 0
        except (subprocess.TimeoutExpired, FileNotFoundError):
            return False

    def compile_with_sanitizer(
        self,
        source_file: str,
        output_file: str,
        ...
        # 添加其他标志
        args.extend([source_file, "-o", output_file])
    ...
    def run_with_sanitizer(
        ...
        timeout: int = 60,
        sanitizer_type: SanitizerType = SanitizerType.ADDRESS,
    ) -> SanitizerResult:
        """
        使用sanitizer运行程序
        ...
        try:
            ...
            return SanitizerResult(
                ...
                warnings=[],
                output=output,
                metadata={"return_code": result.returncode, "timeout": timeout},
            )
    ...
    def _parse_sanitizer_errors(
        self, output: str, sanitizer_type: SanitizerType
    ) -> List[str]:
        """解析sanitizer错误输出"""
        errors = []
        ...
        # LSan错误模式
            ...
            patterns = [r"ERROR: LeakSanitizer: ([^\n]+)", r"detected memory leaks"]
    ...
    def test_with_asan(
        self, source_file: str, output_file: str = "./a.out"
    ) -> SanitizerResult:
        """使用AddressSanitizer测试"""
        # 编译
        success, errors = self.compile_with_sanitizer(
            source_file, output_file, [SanitizerType.ADDRESS]
        )
        ...
        # 运行
        return self.run_with_sanitizer(
            output_file, sanitizer_type=SanitizerType.ADDRESS
        )
    ...
    def test_with_ubsan(
        ...
        # 编译
        success, errors = self.compile_with_sanitizer(
            source_file, output_file, [SanitizerType.UNDEFINED]
        )
        ...
        # 运行
        return self.run_with_sanitizer(
            output_file, sanitizer_type=SanitizerType.UNDEFINED
        )
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            **self.stats,
            ...
            "error_rate": (
                self.stats["error_detected"] / max(1, self.stats["total_runs"])
            ),
        }
