...
from ..parser.ast_nodes import (
    ...
    CharLiteralNode, BoolLiteralNode, IdentifierExprNode,
    IfStmtNode,
)
...
class TypeInferenceEngine:
    ...
    def _infer_if(self, node: IfStmtNode, env: TypeEnv) -> Type:
        ...
        else_type = self.infer(node.else_branch, env) if node.else_branch else BaseType.VOID
    ...
    def _occurs_in(self, var: TypeVariable, type_: Type) -> bool:
        ...
        if isinstance(type_, FunctionType):
            return (any(self._occurs_in(var, t) for t in type_.param_types) or
                    self._occurs_in(var, type_.return_type))
