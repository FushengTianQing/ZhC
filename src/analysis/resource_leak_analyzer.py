...
class ResourceLeakAnalyzer(StaticAnalyzer):
    ...
    # 资源分配函数
    ALLOC_FUNCTIONS = {
        '分配内存', 'malloc', '分配', 'alloc', 'new',
        '打开文件', '打开', 'fopen', 'open', '文件打开',
    }
    
    # 资源释放函数
    FREE_FUNCTIONS = {
        '释放内存', 'free', '释放', 'delete',
        '关闭文件', '关闭', 'fclose', 'close', '文件关闭',
    }
    ...
    def _analyze_function(self, func: Any) -> None:
        ...
        for alloc_id, alloc_node in allocations:
            # 检查是否有对应的释放
            released = any(
                self._resources_match(alloc_id, rel_id)
                for rel_id, _ in releases
            )
    ...
    def _is_allocation(self, node: Any) -> bool:
        ...
        if hasattr(node, 'node_type'):
            type_str = str(node.node_type.value) if hasattr(node.node_type, 'value') else str(node.node_type)
            if 'Call' in type_str:
                if hasattr(node, 'function'):
                    func_name = getattr(node.function, 'name', '')
                    return any(alloc in str(func_name) for alloc in self.ALLOC_FUNCTIONS)
        return False
