...
from zhc.parser.ast_nodes import (
    ASTNode, ASTNodeType,
    VariableDeclNode, IdentifierExprNode,
    FunctionDeclNode, StructDeclNode, ParamDeclNode,
)
...
def find_all_nodes(ast: Any, node_type: Any = None, recursive: bool = True) -> List[Any]:
    """
    查找所有指定类型的节点
...
def get_function_depth(node: Any) -> int:
    ...
    while current:
        ...
        if getattr(current, 'node_type', None) in (
            ASTNodeType.IF, ASTNodeType.WHILE, ASTNodeType.FOR,
